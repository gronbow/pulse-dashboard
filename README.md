...
- 完整模式逐卡显示数据日期与来源；紧凑模式把短日期并入单位行。日期未知、旧数据和缺失值会明确标记，不以顶部快照日期替代各指标自己的日期。
...
| 数据新鲜度 | 取决于 COROS MCP 返回内容和最近一次成功发布的时间；当天未归档指标可能显示带真实日期的最近有效值。当前源码候选会逐项显示日期与 `COROS` / `汇总` / `用户确认` / `演示` 等受控来源标签。旧版插件未写入来源时只能按传输上下文保守推断。 |
...
- 指标来源只接受固定枚举，不允许把工具名、端点、账户、活动 ID 或坐标写入公开展示字段；动态标签和活动列表使用文本节点渲染，本机 JSON 响应禁止 MIME 嗅探。
