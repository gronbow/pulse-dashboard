...
当前公开版本：桌面端与 Codex 插件 v0.5.5 Beta；当前源码：v0.5.6 候选。PR A“训练安全呈现”与堆叠在其上的 PR B“数据可信度呈现”均已创建为 Draft、检查通过且尚未合并或发布。候选 PR C“仓库与页面加载加固”只在本地隔离分支开发，尚未提交或上传。
...
| 页面加载边界 | 候选 PR C 将 Renderer 从 `file://` 迁移到 `pulse-app://dashboard`，仅放行 HTML、CSS 与三个脚本共五个固定资源；IPC 只接受精确入口地址，未知路径和非 GET 请求纳入自动测试。 |
...
1. 完成候选 PR C 的完整回归、实窗截图、解包构建和隐私审计，在提交与上传前进行人工审核。
2. 保持堆叠顺序：先审查并合并 PR A，再更新/审查 PR B，最后基于最新主线处理候选 PR C；发布仍需单独授权。
