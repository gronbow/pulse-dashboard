...
当前版本：桌面端 v0.5.2 Beta，本地发布候选开发中；Codex 插件 v0.5.1 继续兼容
...
| 模块 | 当前状态 |
|---|---|
| 桌面体验 | Dashboard、训练与健康卡片、设置、托盘、主题、透明度、置顶、精简模式及内置圆角滚动条可用；Windows 托盘已改用多尺寸原生 ICO，并保留 PNG 回退。 |
...
| 验证 | 演示 Bridge、快照、适配器、Codex Handoff、发布脚本、隐私审计、真实 Electron 截图和托盘原生图像检查均纳入本地验证。 |
...
核心链路已由“仅在对话中输出文字”调整为“Codex 生成快照 → 本机 Handoff → Pulse 桌面卡片自动更新”。本轮进一步修复了 Windows 托盘空白占位图：桌面端从不受支持的 SVG Data URL 改为随包携带的多尺寸 ICO，启动时会检查空图与关键像素；下一项是人工安装 / 卸载和连续多日复测。

## 下一步

1. 在新 Codex 任务中加载本地 `pulse-dashboard` 插件 v0.5.1 工作流。
2. 在测试机人工安装 / 卸载 v0.5.2 未签名测试包，并人工确认系统托盘显示 Pulse 标志。
