...
看板会逐卡显示数据日期与来源，对早于快照日期的指标显示其真实日期，不把旧值冒充成当天新值。训练计划同样可用 `plan.date` 标记课表所属日期。

指标、活动、计划和负荷可选携带 `provenance`。允许值只有：`coros`（COROS 直接读取）、`derived`（本地算术汇总）、`user`（用户主观确认）、`demo`、`local`、`mixed` 和 `unknown`。不确定时必须使用 `unknown`，不得按数值内容猜测。`meta.source` / `meta.provider` 描述快照如何传到桌面，不等于指标本身来自哪里；来源字段不得包含工具名、接口地址、账户、活动 ID、坐标或其他可识别信息。旧快照没有 `provenance` 时，桌面端只依据受控的传输元数据保守推断。

Handoff 的所有 JSON 响应都使用 `Cache-Control: no-store` 和 `X-Content-Type-Options: nosniff`。页面中的标签和活动内容通过文本节点构建，不把快照字符串作为 HTML 注入。
