...
截至 2026-08-09，仓库已启用 GitHub Vulnerability Alerts、Dependabot Security Updates、Secret Scanning、Push Protection 和 Private Vulnerability Reporting。`main` 要求经 Pull Request 合并、`test` 状态通过并解决讨论；管理员同样受保护规则约束，强推和删除均被禁止。
...
- 工作流先跑完整测试，再构建 unsigned NSIS x64 安装包并验证打包图标与 Electron Fuse。
...
1. 按堆叠顺序审查并合并候选 PR。
2. 在目标远程提交的干净检出中通过 `npm ci`、`npm run test:ci`、`npm run dist:win` 与 `npm run test:packaged`。
